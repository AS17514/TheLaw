using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public enum E_DesireType
{
    Entity1_FilledWithFood,//装满食物
    Entity1_Urgent,//迫切
    Entity1_Feed,//进食
    
    Entity2_HiddenBehindTheClothes,//藏在衣服的后面
    Entity2_IfThatCountsAsMyClothesToo,//如果那也算我的衣服
    Entity2_IfThoseCouldBeSofter,//如果那些****能柔软些
    Entity2_PleaseTearThoseTornTattersApart,//请把那些破布撕碎吧
    Entity2_ThereAreNoMoreWishesLeft,//已经没有再多的愿望了
    
    Entity3_TheWishToStopInTheMiddleOfRunning,//停在奔跑中间的愿望
    Entity3_IWantToLeaveLikeYou,//我想和你一样离开
    Entity3_RunAwayHopingThatYourRunningWonNotBeGivenThatNameAgain,//逃跑，希望自己的奔跑不要再被冠以那样的名字
    
    Entity5_TheirWishes,//“他们”的愿望
    Entity5_Food,//食物
    Entity5_Praise,//称赞
    Entity5_Free,//自由
    Entity5_Happiness,//幸福
    Entity5_WeDonNotNeedHimEitherJustLikeHer//我们也不需要他，和她一样
    
}
