...
public class Part3_1 : Part
{
    ...
    {
        base.Die();
        if (ProgressManager.Instance.nowEntities[0] is Entity entity3)
        {
            if (entity3.GetBuff(E_BuffType.Desire) > 3)
            {
                entity3.AddBuff(E_BuffType.Desire, -3);
            }
        }
    ...
    public static void PartApear()
    {
        ...
        GameObject managerObj = new GameObject("Part3_1");

        // 关键修改 1：把新生成的组件存进一个局部变量 newPart 里
        Part3_1 newPart = managerObj.AddComponent<Part3_1>();
